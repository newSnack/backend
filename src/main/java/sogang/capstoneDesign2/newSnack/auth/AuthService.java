...
import sogang.capstoneDesign2.newSnack.auth.dto.LoginSuccessResponseDTO;
import sogang.capstoneDesign2.newSnack.auth.dto.SignUpDTO;
import sogang.capstoneDesign2.newSnack.auth.vo.TokenRefreshVO;
import sogang.capstoneDesign2.newSnack.user.Provider;
import sogang.capstoneDesign2.newSnack.user.User;
import sogang.capstoneDesign2.newSnack.user.UserRepository;
...
public class AuthService {
  ...
  public ResponseEntity<?> refreshToken(String accessToken, String refreshToken) {
    ...
    Optional<UserRefreshToken> optional = userRefreshTokenRepository.findByUserIdAndRefreshToken(
        userId, refreshToken);
    if (optional.isEmpty()) {
      ResponseMsg message = new ResponseMsg(ResponseMsgList.INVALID_REFRESH_TOKEN.getMsg());
      return ResponseEntity.badRequest().body(message);
    ...
    TokenRefreshVO tokenRefreshVO = TokenRefreshVO.builder().accessToken(newAccessToken.getToken())
        .refreshToken(refreshToken).build();
    // and regenerate refreshToken(if necessary)
    Long leftValidTime =
        refreshAuthToken.getTokenClaims().getExpiration().getTime() - now.getTime();
  ...
  public ResponseEntity<?> registerUser(SignUpDTO signUpDTO) {
    ...
    if (userRepository.findByUserId(signUpDTO.getUserId()).isPresent()) {
      return ResponseEntity.status(HttpStatus.CONFLICT)
          .body(new ResponseMsg(ResponseMsgList.USER_ALREADY_EXISTS.getMsg()));
    }
  ...
  public ResponseEntity<?> logIn(LogInDTO logInDTO) {
    ...
    if (userOptional.isEmpty()) {
      return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
          .body(new ResponseMsg(ResponseMsgList.NO_SUCH_USERID_AND_PASSWORD.getMsg()));
    }
    ...
    // create access token
    AuthToken accessToken = authTokenProvider.createAccessToken(user.getId().toString(),
        user.getRoleType().getCode());
    // create refresh token and update if needed
    AuthToken refreshToken = authTokenProvider.createRefreshToken(user.getId().toString(),
        user.getRoleType().getCode());
    Optional<UserRefreshToken> optional = userRefreshTokenRepository.findByUserId(
        user.getId().toString());
    if (optional.isPresent()) {
      ...
      UserRefreshToken userRefreshToken = UserRefreshToken.builder().userId(user.getId().toString())
