...
import sogang.capstoneDesign2.newSnack.auth.*;
...
public class OAuth2AuthenticationSuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
  ...
  public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
    ...
    clearAuthenticationAttributes(request,
        response); // 소셜 authentication 정보를 지워주고 이후 토큰으로 필터 시 authentication을 설정함.
    getRedirectStrategy().sendRedirect(request, response,
        targetUrl);   // 현재 request에 대한 응답을 targetUrl로 redirect
  }
  ...
  protected String determineTargetUrl(HttpServletRequest request, HttpServletResponse response,
    ...
    Optional<UserRefreshToken> userRefreshTokenOptional = userRefreshTokenRepository.findByUserId(
        userId);
