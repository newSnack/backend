...
import sogang.capstoneDesign2.newSnack.auth.info.OAuth2UserInfo;
import sogang.capstoneDesign2.newSnack.auth.info.OAuth2UserInfoFactory;
...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
  ...
  private OAuth2User process(OAuth2UserRequest userRequest, OAuth2User user) {
    ...
    OAuth2UserInfo userInfo = OAuth2UserInfoFactory.getOAuth2UserInfo(provider,
        user.getAttributes());
    Optional<User> oldUser = userRepository.findByProviderAndSocialId(provider,
        userInfo.getId()); // provider & social id로 검색
    User principal;
    if (oldUser.isEmpty()) {
